...
void CurveWnd::modifyChannelDataAtPosition(int channelIndex, quint16 startPos, int count, double value)
{
    if (channelIndex >= 0 && channelIndex < seriesdata.size()) {
        auto& raw_y = seriesdata[channelIndex]->raw_points.y;
        int size = raw_y.size();

        // 从 startPos 开始，修改接下来的 count 个数据点
        for (int i = 0; i < count && (startPos + i) < size; ++i) {
            int index = (startPos + i) % max_point_num;  // 处理循环缓冲区
            raw_y[index] = value;
        }

        seriesdata[channelIndex]->rescale();
        update();
    }
}
